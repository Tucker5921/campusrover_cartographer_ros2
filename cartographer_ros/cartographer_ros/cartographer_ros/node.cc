...
int Node::AddTrajectory(const TrajectoryOptions& options,
  ...
  std::cout<<"AddTrajectory : node.cc"<<std::endl;
...
bool Node::HandleMapLoad(
  ...
  // carto::common::MutexLocker lock(&mutex_);
  LoadState(request.filename, true);
  response.status.code = cartographer_ros_msgs::StatusCode::OK;
  response.status.message = "susessful load map'" + request.filename + "'.";
  return true;
}
...
bool Node::HandleDeleteTrajectory(
  ...
  is_active_trajectory_[request.trajectory_id] = false;
  const std::string message =
    "Deleted trajectory " + std::to_string(request.trajectory_id) + ".";
  response.status.code = cartographer_ros_msgs::StatusCode::OK;
